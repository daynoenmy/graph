...
The script follows the current repository's medical evaluation rule: an image
score is the maximum value of its pixel anomaly map. It evaluates AA-CLIP and
V1 on exactly the same input tensor, writes per-case statistics, selects
representative (median rather than best) cases, and creates paper-ready panels.
...
def patch_anomaly_map(patch_features, text_embeddings, img_size, dataset):
    ...
    return calculate_similarity_map(
        patch_features,
        text_embeddings,
        img_size,
        test=True,
...
def finalize_case_records(records, bounds):
    for name in ("aa", "v1"):
...
def write_analysis_info(
    ...
    lines = [
        ...
        "",
        "Medical image scores are dataset-normalized pixel-map maxima, matching forward_utils.metrics_eval.",
...
def parse_args():
    ...
    parser.add_argument("--v1_save_path", type=str, default="ckpt/noise_graph")
    parser.add_argument(
...
def main():
    ...
    records = finalize_case_records(records, bounds)
